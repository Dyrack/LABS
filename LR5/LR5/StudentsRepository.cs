        ...
        public StudentsRepository(string fileName)
        {
            ...
            using(StreamReader streamReader = new StreamReader(fileName))
            {
                ...
                while(!streamReader.EndOfStream)
                {
                    ...
                    Student student = new Student(mass[1], mass[2], int.Parse(mass[3]), double.Parse(mass[5]));
                    //добавляем студента в университет 
                    this.university.Add(student);
                    //проставляем номер зачетки студента из 4 ячейки массива 
                    student.Number = int.Parse(mass[4]);
                }
            }
        }
